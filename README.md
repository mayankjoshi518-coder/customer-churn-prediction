...
**Tools:** Python (pandas, scikit-learn, matplotlib, seaborn) · SQL (sqlite3) · Jupyter Notebook
...
3. **Monthly charges** — Customers paying >£70/month without bundled services churn 2.8× more
...
├── notebooks/
│   └── churn_pipeline.ipynb    # Full pipeline: SQL → ML → business output
├── data/
│   └── telco_churn_sample.csv  # 200-row sample (full data from Kaggle link above)
├── visualisations/
...
jupyter notebook notebooks/churn_pipeline.ipynb
```

> Download the full dataset from Kaggle and save as `data/telco_churn.csv`
